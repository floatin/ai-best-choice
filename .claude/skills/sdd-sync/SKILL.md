...
- **操作**:
    ...
    2. 执行 `scripts/git-sync.sh init`。 (该步骤的 Git 操作逻辑参见 [references/git-flow.md#1-基础配置与仓库初始化-initialization])
...
- **操作**:
    1. 运行 `scripts/checkpoint.sh save`。
...
- **目标**: 整理代码并提交 PR 准备。
- **操作**:
    1. 运行 `scripts/git-sync.sh finalize` 同步至 `upstream`。
